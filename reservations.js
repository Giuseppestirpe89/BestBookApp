...
function confirmSeats(){
	...
	var rdate = document.getElementById("datebtn").value;
	var remail = document.getElementById("emailbtn").value;
	var rtime = document.getElementById("timebtn").value;
	if(name == "" || tnum == "" || nseats == "" || rid == ""|| rdate == ""|| remail == "" || rtime == ""){
		return false;
	}
	
	// Compile our data from form to send to processing
	var confData = "confirm="+rid+"&n="+name+"&tn="+tnum+"&ns="+nseats+"&datebtn="+rdate+"&emailbtn="+remail+"&timebtn="+rtime;
    ...
    hr.onreadystatechange = function() {
	    if(hr.readyState == 4 && hr.status == 200) {
			...
			if (return_data[0] == "false"){
				...
				alert ("You book the seats "+name+ ". Thank you very much for your booking. See you the "+rdate+" at"+rtime+". A copy of your reservation it has been send to "+remail+"");
				// Reload the page
