...
if (isset($_POST['getSeatBtns'])){
	...
	if ($quick_check != 0){
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
			...
			$spots .= $tablenum.' now has '.$avail.' seat(s) available - <br />';
			$spots .= "Click number of seats you want to reserve, will be held for 3 minutes once you click.<br /><br />";
...
if (isset($_POST['reserve'])){
	...
	$date = preg_replace("([^0-9/])", '', $_POST['datebtn']);
	$email = preg_replace('#[^A-Za-z@0-9]#i', '', $_POST['emailbtn']);
	$time = preg_replace("([^0-9/])", '', $_POST['timebtn']);
	...
	if ($quick_check != 0){
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
			...
			$spots .= 'You only have 3 minutes to finish or your reservation expires and the seats open up to other people.<br /><br />';
		}
		...
		//name
    	$spots .= 'Name : <input id="name" type="text">';
    	//date field
    	$spots .= '&nbsp;&nbsp; Date : <input id="datebtn" type="date" value="'.$date.'"><br /> <br />';
    	//time field
    	$spots .= 'Time : <input id="timebtn" type="time" value="'.$time.'">';
    	//email field
    	$spots .= '&nbsp;&nbsp;&nbsp; Email : <input id="emailbtn" type="email" value="'.$email.'"><br /><br />';
		...
		$spots .= '<button id="confirmbtn" class="btn btn-info" onClick="confirmSeats()">Book Seats</button>';
...
if (isset($_POST['confirm'])){
	...
	$time = preg_replace("([^0-9/])", "", $_POST['timebtn']);
	// If POSTS are empty after cleaning, exit
	if($rid == "" || $name == "" || $tableNumber == "" || $numSeats == "" || $date == "" || $email == "" || $time == ""){
		exit();
	}
	...
	if ($quick_check != 1){
		...
		} else 
		{
			...
			$sql = "INSERT INTO confirms(tablename,numseats,person,date,email,time) VALUES ('$tableNumber','$numSeats','$name','$date','$email','$time')";
			$query = mysqli_query($connect, $sql);
			// Build display output
